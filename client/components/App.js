...
const PersonForm = ({
  ...
  <form onSubmit={submitHandler}>
    <div>
      name: <input value={newNameState} onChange={nameChangeHandler} />
    </div>
    <div>
      number: <input value={newNumberState} onChange={numberChangeHandler} />
    </div>
    <div>
...
const App = () => {
  ...
  const showIfVisible = (person) => {
    const show = person.name.toLowerCase().includes(filterValue.toLowerCase());
    if (show) {
  ...
  const updatePerson = (newPerson) => {
    ...
    personService
      ...
      .then((returnedPerson) => {
        ...
        setNewName('');
        setNewNumber('');
        setNotification(`Updated number for ${returnedPerson.name}`, 'success');
